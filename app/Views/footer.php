    ...
    <div class="container cont-width">
        <div class="row">
            <div class="col-lg-3 col-md-3 col-12 footer-div flid">
                ...
                <p class="mt-2 tx">At Enatyam, we are providing you with professional training of dance forms
                    originating
        ...
        <div class="row">
            <div class="col-md-1 col-12 icn">
                <div class="social-div ">
                    ...
                    <a href="https://wa.me/919272122001 " class="insta-div"><img
                            src="<?=base_url(); ?>public/images/whatsapp.png" /></a>

                    <a href="https://www.instagram.com/enatyam.india/" class="insta-div"><img
                            src="<?=base_url(); ?>public/images/instagram.png" /></a>

                    <a href="https://www.linkedin.com/company/enatyam/" class="twitter-div"><img
            ...
            <div class="col-md-11 col-12 ">
                ...
                <div class="copyright-p ">
                    <p> 2024 © All Rights are reserved | <a href="https://www.marketingintelligence.tech"> Enatyam India
                            Pvt. Ltd. </a>
                    </p>
...
function handleSubmit(collapse_val) {
    ...
    if (collapse_val == "collapseOne") {
        ...
        $("#namespanid").html(name_error_msg);
        $("#emailspanid").html(email_error_msg);
        $("#phonespanid").html(phone_error_msg);
        $("#coursespanid").html(demo_for_error_msg);
        $("#sub_coursespanid").html(demo_for_error_msg);
    ...
    $(document).ready(function() {
        ...
        $("input[name='exprience']").on("change", function() {
            var selectedValue = $("input[name='exprience']:checked").val();

            // Check the selected value and toggle the visibility accordingly
            if (selectedValue === "Yes") {
                // Show the section if "Yes" is selected
                $(".exprience-details").show();
            } else {
                // Hide the section if "No" is selected
                $(".exprience-details").hide();
            }
        });
    });
                    ...
                    if (response !== "No address found for this pincode.") {
                        ...
                        if (addresses.length > 0) {
                            ...
                            addresses.forEach(function(addressObj) {
                                ...
                                // $imageDiv.append('<img src="http://localhost/enatyam/public/Offilinstudio/' + imageName + '" alt="Address Image">');
                                var imageName = addressObj.img; // Image for each address
                                $imageDiv.append(
                                    '<img src="<?= base_url('public/Offilinstudio/') ?>' +
                                    imageName + '" alt="Address Image">');
                                // Text on the other side
                                var $textDiv = $('<div></div>').addClass('text');
                                $textDiv.append('<h3>' + name + '</h3>');
                                $textDiv.append('<p>' + address + '</p>');
                                $textDiv.append('<p><b>Contact No:</b> ' + contact +
                                '</p>');
                                $textDiv.append('<p><b>Google Location:</b> <a href="' +
                                    googleLocation + '">' + googleLocation + '</a></p>');
...
// Step-1 validation
{
    ...
    if (collapse_val == "collapseOne") {
        ...
        if ($("#code_mobileNo").val()) {
            if (!phoneno_regx.test($("#code_mobileNo").val())) {
                phone_error_msg = "Please enter valid digits phone number.";
            } else {
                phone_error_msg = "";
            }
        } else {
        ...
        $("#namespanid").html(name_error_msg);
        $("#emailspanid").html(email_error_msg);
        $("#mobile_noError").html(phone_error_msg); // Update the phonespanid error message
        $("#coursespanid").html(demo_for_error_msg);
        $("#subcoursespanid").html(demo_for_error_msg);
    ...
    if (collapse_val == "collapseFour") {
        ...
        if (!education) {
            $("#educationError").html("Please select an education level");
        ...
        if (!experience) {
            $("#experienceError").html("Please select an experience level");
            return; // Prevent further processing
        }
    ...
    $("form").submit(function(event) {
        ...
        var selectedDate = $("#demo_class_date").val();
        if (!moment(selectedDate, "YYYY-MM-DD").isValid()) {
            $("#demo_class").text("Please enter a valid date.");
            valid = false;
        }
...
setInputFilter(document.getElementById("txtMobileNo"), function(value) {
    return /^\d*$/.test(value);
}, "Must be a number");
                    ...
                    success: function(response) {
                        ...
                        if (response.email.status == '203') {
                            ...
                            if (response.email.emailexist == true) {
                                ...
                                $('#emailError').removeClass('d-none').text(
                                    'Email already exist.')
        ...
        if (countryId) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#sub_courses_id_g').append(
                        '<option value="">Please select sub Courses</option>');
                    $.each(data, function(key, value) {
                        $('#sub_courses_id_g').append('<option value="' + value.id +
                            '">' + value.sub_courses_name + '</option>');
                    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response === 'false') {
                    ...
                    $('#demoMobileNoError').text(
                    'This mobile number is already available.');
                    $('.submitButton').prop('disabled', true);
    ...
    otpButton.addEventListener('click', function () {
        if (otpOption.checked) {
            ...
            fetch('<?php echo base_url(); ?>loginwithotp', {
                ...
                body: `mobilenumber=${encodeURIComponent(combinedMobile.value)}`,
            })
